...
fn encrypt(mk: &MessageKey, plaintext: &[u8], associated_data: &[u8]) -> Vec<u8> {
    let ikm = mk;
    let salt: [u8; 32] = [0; 32];
...
fn decrypt(mk: &MessageKey, ciphertext: &[u8], associated_data: &[u8]) -> Result<Vec<u8>, &'static str> {
    let ikm = mk;
    let salt: [u8; 32] = [0; 32];
    let info = hex!("734f73456e6372797074"); // 'sOsEncrypt'
...
mod tests {
    ...
    use crate::{init_all, protocol::{State, encrypt, decrypt, generate_dh, dh, Header, MAX_SKIP}, ratchet_encrypt, ratchet_decrypt};
