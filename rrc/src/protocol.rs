...
pub fn rrc_init_all(security_level: Security) -> (RrcState, RrcState) {
    ...
    let alice_state = RrcState{state: alice_rc_state, hash_key: hash_key.to_bytes().clone(), hash_key_prime: hash_key_prime.to_bytes().clone(), s: HashSet::new(), r: HashSet::new(), s_ack: HashSet::new(), max_num: Ordinal { epoch: 0, index: 0 }, security_level: security_level.clone()};
    let bob_state = RrcState{state: bob_rc_state, hash_key: hash_key.to_bytes(), hash_key_prime: hash_key_prime.to_bytes(), s: HashSet::new(), r: HashSet::new(), s_ack: HashSet::new(), max_num: Ordinal { epoch: 0, index: 0 }, security_level: security_level};
...
fn checks(state: &mut RrcState, ct: &mut Ciphertext, h: &[u8; 32], num: Ordinal) -> bool {
    ...
    if num < state.max_num {
        ...
        state.max_num = num;
        r_bool = r_bool || state.s_ack.difference(&ct.s).into_iter().fold(false, |acc, msg| acc || msg.ordinal < state.max_num); // -> fix w.r.t paper
    }
 ...
 */
pub(crate) fn incremental_hash_fct_of_whole_set(r: &HashSet<Message>, hash_key_prime: &[u8; 32]) -> [u8; 32 + 2 * M_BYTES] {
...
/*Hash function 0 is SHA256, Hash function 1 is Blake2s256 */
pub(crate) fn incremental_hash_sets_are_equal(hash1: [u8; 32 + 2 * M_BYTES], hash2: [u8; 32 + 2 * M_BYTES], hash_key_prime: &[u8; 32]) -> bool {
...
pub(crate) fn update_incremental_hash_set(incremental_hash: &mut [u8; 32 + 2 * M_BYTES], msg: Message, hash_key_prime: &[u8; 32]) -> [u8; 32 + 2 * M_BYTES] {
...
pub fn receive_bytes(payload: &[u8], state: &mut RrcState, associated_data: &[u8; 32]) -> (bool, Ordinal, Vec<u8>) {
    // 1. Decode header
    ...
    let mut ct = Ciphertext { ciphertext: payload[ct_offset..ct_offset + ct_len].to_vec(), s, r: (r_0, r_1) };
